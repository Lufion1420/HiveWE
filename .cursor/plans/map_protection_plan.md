...
**Update: root cause found and fixed - `AbilityBuffMetaData.slk` classifies buff SFX fields as `type="modelList"`, a third type value the pipeline never accounted for.** Confirmed directly from the user's own console output: added a temporary diagnostic print in `rewrite_object_data_file()` for every `war3map.w3h` field, rebuilt, and had the user re-run the export against their real map. Result: `targetart`/`specialart` resolve to `meta_id`s whose `type` column is literally `modelList`, not `model` - `buffart` correctly resolves to `icon` (already working, which is why only some buff SFX were broken and not others). `rewrite_object_data_file()`'s type check (`if (type != "model" && type != "icon") continue;`) had no branch for this value at all, so these fields were silently skipped exactly like any other non-path field - not a bug in `field_to_meta_id()` itself, just an incomplete type check one level up.

**Fix**: extended the check to also accept `type == "modelList"`. Every real value observed so far is a single path, but the type name implies the field format allows a comma-separated list (consistent with every other `*List`-typed field in this codebase - `unitList`/`abilityList`/`buffList`/etc. in `src/models/table_model.ixx` - which are all comma-separated and split via `absl::StrSplit`/joined via `absl::StrJoin`), so the fix splits on `,` (preserving empty segments for exact round-trip fidelity when nothing matches), rewrites whichever segments match a candidate, and rejoins only if at least one segment changed - a single-path value round-trips as a "list" of one element with zero behavioral difference from before. 1 new test (`rewrite_object_data_file rewrites a modelList-typed field, including a comma-separated value` in `tests/asset_obfuscation_test.cpp`) covering a two-segment value where only the first segment matches a candidate. Full suite: 55 passed, same 5 pre-existing unrelated failures. `HiveWE.exe` and `HiveWE_tests.exe` both built clean.

**Next steps, in order:**
1. User re-test against the real failing map (`release.w3x`) with Asset Path Obfuscation on: confirm export succeeds, output is smaller than source, loading screen and custom UI render correctly, and specifically the buffs that were broken (Stunned, AmaterasuMedium, FireBurnTargetHQ) show their correct effects in-game.
2. Also re-run a bare-minimum export (everything off) to confirm the error-87 crash is gone.
3. Ladik's MPQ Editor: confirm renamed files still don't resolve to recognizable names.
4. Combined with `strip_trigger_strings`/`encrypt_files`/`inject_junk_files` together.
5. `NCOW_SFX_Chidori.mdx`/`.mdl` and the `NCOW_Model_*` entries noted missing during an earlier reachability scan were never confirmed as tracked in `war3map.imp` (unlike FireBurnTargetHQ/Stunned/AmaterasuMedium, which were) - watch for whether these are still broken after the `modelList` fix, since they may be a separate, unrelated issue (e.g. a pre-existing broken reference in the original map, or a reference without a file extension).
