...
Next up per the staged plan: Phase 5 (script literals - reuses `find_string_literals()`/`escape_script_string()` from `protection_pipeline.ixx`), then Phase 6 (verify nothing was missed + physically rename the files + wire everything into `run_async_pack()`).
