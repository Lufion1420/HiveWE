...
1. Re-run the exact failing case (`NCOW_Arena.w3x`, "Rename imported files to random names" on, everything else default) with all fixes and confirm: the export succeeds cleanly (no crash, no desktop.ini abort), the loading screen and custom UI render correctly in-game.
...
3. In-game: full playability pass - a model with attachments, a custom-icon unit, a doodad, a script special-effect.
