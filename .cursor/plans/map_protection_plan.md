...
## Status: feature-complete, not yet in-app or in-game verified

All 6 phases are implemented, wired into the real export path, and build clean. What's still needed before trusting this on a real map:
1. **In-app**: protect a real map (ideally the user's actual external-source `.w3x` workflow, since that's what this was built for) with "Rename imported files to random names" enabled, then reopen the protected output in HiveWE itself and confirm everything still resolves (models render, icons load, no missing-file spam).
2. **In-game**: load the protected map in a real Warcraft III client, confirm full playability - specifically exercise a model with attachments, a unit with a custom icon, a doodad, and anything referencing a special-effect path from script.
3. **Tool-level**: open the protected output in Ladik's MPQ Editor (or equivalent) and confirm renamed files no longer resolve to recognizable names - the actual point of this feature.
4. **Combined-options regression**: enable this alongside `strip_trigger_strings`, `encrypt_files`, and `inject_junk_files` together, since asset-path rewriting and TRIGSTR stripping both touch `war3map.j`/`war3map.lua` in the same pipeline run.
