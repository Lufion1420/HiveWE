...
**Real architectural question surfaced during implementation, not resolved yet:** Phase 2 (object-data SLK field rewriting) needs a fully set-up template+meta SLK per category (e.g. `units_slk`/`units_meta_slk`), but `Map::load()` (`src/base/map/map.ixx:110-350ish`) builds these via the stock `.slk` **plus many `.merge(ini::INI(...))` calls** (skin/weapons-func/strings files per race) and `build_meta_map()` — this full setup currently only happens inside `Map::load()`, tied to the global `map` singleton. For the "currently loaded map" protection path this is trivial (reuse `map->units_slk` etc. directly), but for the **external source path** (`prepare_source_path()` — protecting a `.w3x`/folder that was deliberately never loaded into HiveWE's `Map` object, precisely to avoid clobbering externally-compiled scripts, see this doc's own "Architecture (as built)" section above) there is no equivalent already-built table to reuse. Needs a decision before Phase 2 lands: (a) only support asset obfuscation for the currently-loaded-map path, (b) extract Map::load()'s per-category SLK setup into a shared loader both `Map::load()` and this pipeline step call (real refactor of core map-loading code — higher risk, touches load/save sequencing this repo's CLAUDE.md explicitly flags as sensitive), or (c) build a lighter, possibly-incomplete standalone loader for this purpose only, accepting that a resolution failure aborts the export (safe) rather than silently doing the wrong thing. Not decided yet — raise with the user before starting Phase 2.
