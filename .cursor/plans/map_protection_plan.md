...
1. Re-run the exact failing case (`release.w3x`, "Rename imported files to random names" on, everything else default) with all fixes and confirm: the export succeeds cleanly (no crash, no abort), the loading screen and custom UI render correctly in-game, and SFX/effects using single-quoted Lua calls play correctly.
