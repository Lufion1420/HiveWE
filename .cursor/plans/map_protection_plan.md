...
Next up per the staged plan: Phase 3 (`war3map.w3i` - smallest remaining reference kind, good candidate to validate the full loop end-to-end once Phase 6 exists), then Phase 4 (MDX-internal paths), Phase 5 (script literals), Phase 6 (verify + wire everything into `run_async_pack()`, including the physical rename).
